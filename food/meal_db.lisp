...
(defun request-mealDB (meal &optional (api-key *api-key-meal*))
 "Search for a Recipe in the MealDB"
  (let ((url (format nil "https://www.themealdb.com/api/json/v1/1/search.php?s=~a" meal api-key)))
    (request-api url)))

(defun request-mealDB-filter (&key (api-key *api-key-meal*) ingredient category country)
...
(defun request-mealDB-hungry-random(&optional (api-key *api-key-meal*))
